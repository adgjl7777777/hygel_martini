...
from hydrogel_builder.core_utils.templates.monomer_loader import BeadTemplate
...
def _extract_definition(itp_path: str, molecule_name: str | None) -> Dict:
    ...
    definitions = read_itp_definitions(itp_path, atom_type_masses=mass_map)
    if not definitions:
...
def _load_single_linker(entry: Dict, backbone_defs: List[Dict]) -> LinkerTemplate:
    ...
    backbone_map = {idx: 'dummy_id' for idx in stub_indices}
    ...
    coords = np.array(coords_local, dtype=np.float64)
