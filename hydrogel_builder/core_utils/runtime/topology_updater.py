...
def create_system_topology(output_dir, top_path, itp_files):
    ...
    # Start the ordered list of ITPs, ensuring the base file is absolute and first.
    ordered_itps = [os.path.abspath(base_itp)]

    # Combine other ITPs, avoiding duplicates.
    try:
        additional_itps = Config.get_param('additional_itp_files')
    except KeyError:
        additional_itps = []
        
    other_itps = additional_itps + itp_files
    for itp_file in other_itps:
        abs_path = os.path.abspath(itp_file)
        if abs_path not in ordered_itps:
            ordered_itps.append(abs_path)

    # Build the topology content with the correct include order.
    top_content = ""
    for itp_path in ordered_itps:
