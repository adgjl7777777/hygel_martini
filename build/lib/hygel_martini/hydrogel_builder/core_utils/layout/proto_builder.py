...
class BackboneSequenceFactory:
    ...
    def __init__(self,
        ...
        # Block copolymers are sequence-specific objects, so every placed chain
        # must start at the first block unless the config explicitly asks for a
        # phase shift.  Alternating patterns keep the historical shifting
        # behavior by default.
        self.advance_pattern_offset = bool(
            self.strategy.get('advance_pattern_offset', self.strategy_name != 'block')
        )
    ...
    def next_sequence(self) -> List[Dict[str, Any]]:
        ...
        if self.pattern:
            ...
            if self.advance_pattern_offset:
