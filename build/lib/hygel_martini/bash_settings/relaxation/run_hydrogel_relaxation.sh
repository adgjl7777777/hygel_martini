...
if [ "$WORKFLOW_HELP" -eq 1 ]; then
  cd "$SCRIPT_DIR"
  require_python_module "hygel_martini.hydrogel_builder.relax"
  "$PYTHON_BIN" -m hygel_martini.hydrogel_builder.relax --help
  exit 0
fi
...
cd "$SCRIPT_DIR"
require_python_module "hygel_martini.hydrogel_builder.relax"
"$PYTHON_BIN" -m hygel_martini.hydrogel_builder.relax --config "$CONFIG_PATH"
