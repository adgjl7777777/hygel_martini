...
- `qm_to_martini`
  ...
  - `analysis/reference_qualification.py`
    xTB와 고수준 참조의 상대에너지, gradient, endpoint family,
    reweighting overlap을 분리 판정
- `bead_generator`
  00 단계 예정 위치
- `polymer_maker`
...
고수준 참조가 실제 보정에 사용 가능한지 먼저 검사하려면 다음을
사용합니다.

```bash
hygel-qm-reference-audit --help
hygel-qm-reference-audit energy reference_energies.csv \
  --group-column chemistry \
  --output reference_energy_audit.json
```

이 분석은 fitting을 수행하지 않습니다. 각 gate를 통과한 뒤에만
보정 항의 자유도, training/test 분리, solution-state validation을
별도의 refinement 계획으로 확정해야 합니다.
