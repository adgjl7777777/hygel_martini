...
class Hydrogel():
    ...
    def construct_atoms(self):
        ...
        try:
            ...
            def get_backbone_generator(strategy_info, backbones):
                if strategy_info['strategy'] == 'random':
                    ratios = [b.get('ratio', 1) for b in backbones]
                    def random_generator():
                        while True:
                            yield random.choices(backbones, weights=ratios, k=1)[0]
                    return random_generator()
                else: # Default to cycle
                    return itertools.cycle(backbones)
            
            backbone_generator = get_backbone_generator(bb_strategy, backbone_list)
        ...
        for bx, by, bz in pd(range(self.x_number_of_repeat),
            ...
            for i in segment_xyz:
                ...
                for ii in i:
                    ...
                    chosen_backbone = next(backbone_generator)
