...
# launcher_utils.sh 탐색: 상대 경로 → 패키지 설치 경로 순
...
# 환경 설정 (environment.sh가 있으면 자동 로드)
setup_hygel_env "$SCRIPT_DIR"

# GROMACS 설정 — 하드코딩 없음, 환경 변수로 재정의 가능
...
Shell environment:
  environment.sh 가 같은 디렉터리에 있으면 자동으로 로드됩니다.
  ENVIRONMENT_FILE, PYTHON_BIN, GMX_CMD, GMXRC_PATH 환경 변수로 재정의 가능.
  GMX_BIN는 property_extract.yaml의 gromacs.executable 또는 GMX_BIN 환경 변수로 제어.
EOF
}
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --check-gmx)
      CHECK_GMX=1; shift ;;
    --extract-only)
      EXTRACT_ONLY_FLAG="--extract-only"; shift ;;
    *)
      break ;;
  esac
done
