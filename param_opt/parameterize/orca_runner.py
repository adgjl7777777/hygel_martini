...
from ..structure.maker import load_monomer_library, _sequence_output_stem
...
def generate_orca_inputs(cfg: Dict[str, Any]) -> None:
    ...
    dft_cfg = cfg.get("dft", {
        ...
        "nprocs": 16, "charge": 0, "multiplicity": 1, "max_iter": 300
    })
...
def _build_atoms_for_dft(sequence, monomer_dict, n_torsion):
    ...
    """
    import numpy as np
    from ..structure.maker import get_connection_info, cap_ends_with_hydrogen
...
def _write_orca_file(name, atoms, dft_cfg, out_root):
    ...
    header += "%pal\n"
    header += f"   nprocs {dft_cfg.get('nprocs', 16)}\n"
    header += "end\n"
    header += "%geom\n"
