...
from ..structure.maker import build_polymer, load_monomer_library

from ..core.defaults import NM_TO_ANGSTROM
...
def build_cases(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for seq_tokens in sequence_jobs:
        ...
        tmp_out_name = f"{seq_name}_constructor.xyz"
        build_polymer(seq_tokens, monomer_dict=library, n_torsion=n_torsion, output_filename=tmp_out_name)

        built_path = Path("output") / tmp_out_name
        atoms = read(built_path)
        ...
        shutil.copyfile(built_path, polymer_xyz)
