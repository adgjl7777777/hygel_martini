...
def load_monomer_library(monomer_files, base_dir=None):
    """
    monomer_files:
      {"S": "NEW_SBMA.xyz", "D": "NEW_DMAPS.xyz"} 또는
      {"S": {"xyz": "NEW_SBMA.xyz"}, ...} 형태
    """
    if base_dir is None:
    ...
    monomer_dict = {}
    for symbol, raw_entry in monomer_files.items():
        filepath = raw_entry.get("xyz") if isinstance(raw_entry, dict) else raw_entry
        if not filepath:
            raise ValueError(f"모노머 '{symbol}'의 xyz 경로가 비어 있습니다.")
...
def build_polymer(sequence, monomer_dict, n_torsion, output_filename=None, output_dir="output"):
    """
    Args:
    ...
    write(output_path, final_polymer)
